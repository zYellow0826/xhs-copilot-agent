...
- **Tailwind CSS 3** — 样式
- **@supabase/supabase-js** — 浏览器侧 Supabase 客户端（v0.5 登录用）
...
| 变量 | 用途 |
|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | 浏览器直连 Supabase（v0.5 起登录用） |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Supabase Anon Key（公开可见 OK） |
...
后端 `/generate` 返回 `text/event-stream`，每条事件是 LangGraph `astream_events(v2)` 的 JSON。
...
4. 根据 `event.event` 字段更新进度（"启动 workflow" / "调用 DeepSeek" / "结构化完成" 等）
5. 从 `event.event === 'on_chain_end' && event.name === 'LangGraph'` 抓最终 `output`

[app/api/generate/route.ts](app/api/generate/route.ts) 是 Next.js Route Handler，把请求透传给后端并把流原样回传给浏览器，加了 `x-accel-buffering: no` 防止反向代理缓冲。
...
4. 加环境变量（同 `.env.local.example`），其中 `API_BASE_URL` 填 Railway 给的后端域名
5. SSE 在 Vercel Edge / Node Runtime 都可以，本仓库使用 Node Runtime（`runtime = "nodejs"`，见 route.ts）
...
- `npm run typecheck` 跑 `tsc --noEmit`，CI 推荐加上
